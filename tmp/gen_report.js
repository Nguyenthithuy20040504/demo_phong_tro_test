...
async function check() {
  try {
    ...
    let out = 'USER: ' + user.email + '\n';
    out += 'USER EXPIRY: ' + user.ngayHetHan + '\n\n';
    out += 'PAYMENTS:\n';
    payments.forEach(p => {
        out += `${p.maDonHang} | ${p.trangThai} | Expiry: ${p.ngayHetHanMoi} | Created: ${p.createdAt}\n`;
    });

    fs.writeFileSync('c:/Users/Az09.Team/Git-projects/demo_phong_tro_test/tmp/payment_report.txt', out);
    console.log('Report written to tmp/payment_report.txt');
    process.exit(0);
  } catch (err) {
    console.error(err);
    process.exit(1);
