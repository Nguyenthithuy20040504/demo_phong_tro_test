...
export async function POST(request: NextRequest) {
  try {
    ...
    const tenantEmail = khachThue.email as string;
    const tenantName = khachThue.hoTen as string;

    // Link dẫn tới trang thiết lập mật khẩu dành cho khách
    const confirmLink = `${baseUrl}/thiet-lap-mat-khau?token=${token}&email=${encodeURIComponent(tenantEmail)}`;
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        tenDangNhap: khachThue.tenDangNhap as string,
      }
    }, { status: 200 });
