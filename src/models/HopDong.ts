...
export interface IHopDong extends Document {
  ...
  trangThai: 'choDuyet' | 'hoatDong' | 'hetHan' | 'daHuy' | 'choDuyetGiaHan' | 'choDuyetHuy';
...
const HopDongSchema = new Schema<IHopDong>({
  ...
  trangThai: {
    type: String,
    enum: ['choDuyet', 'hoatDong', 'hetHan', 'daHuy', 'choDuyetGiaHan', 'choDuyetHuy'],
    default: 'choDuyet'
  },
  daNhacChoDuyet: {
    type: Boolean,
...
HopDongSchema.index({ nguoiDaiDien: 1, trangThai: 1 });
HopDongSchema.index({ khachThueId: 1, ngayBatDau: -1 });
